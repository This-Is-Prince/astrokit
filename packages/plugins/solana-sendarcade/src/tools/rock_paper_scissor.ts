...
import { AstroKit, signOrSendTX, SolanaWalletBase } from "astrokit";
...
async function won(agent: AstroKit<SolanaWalletBase>, href: string): Promise<string> {
  try {
    const res = await fetch(
...
async function postWin(agent: AstroKit<SolanaWalletBase>, href: string): Promise<string> {
  try {
    const res = await fetch(
