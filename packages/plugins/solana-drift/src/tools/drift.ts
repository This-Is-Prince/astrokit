 ...
 * @param agent
 */
export async function doesUserHaveDriftAccount(agent: AstroKit<SolanaWalletBase>) {
  try {
    const { driftClient, cleanUp } = await initClients(agent);
    const user = new User({
      driftClient,
 ...
 * @returns
 */
export async function driftUserAccountInfo(agent: AstroKit<SolanaWalletBase>) {
  try {
    const { driftClient, cleanUp } = await initClients(agent);
