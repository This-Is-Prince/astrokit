...
async function getOrCreateVaultDepositor(agent: AstroKit<SolanaWalletBase>, vault: string) {
...
async function getVaultAvailableBalance(agent: AstroKit<SolanaWalletBase>, vault: string) {
  try {
...
*/
async function getIsOwned(agent: AstroKit<SolanaWalletBase>, vault: string) {
  try {
    const { vaultClient, cleanUp } = await initClients(agent);
    const vaultPublicKey = new PublicKey(vault);
    const vaultDetails = await vaultClient.getVault(vaultPublicKey);
 ...
 * @param name
 */
export async function getVaultAddress(agent: AstroKit<SolanaWalletBase>, name: string) {
