...
export interface AstroKitConfig {
  [key: string]: any;
}
 ...
 */
export class AstroKit<W extends WalletBase> {
  ...
  /**
   * Create a new AstroKit instance
   * 
  ...
  constructor(
    ...
    public readonly config: AstroKitConfig = {}
  ) {}
   ...
   * @returns The AstroKit instance with the plugin's methods
   */
  use<P extends Plugin<W>>(plugin: P): AstroKit<W> {
    // Check if plugin is already registered
    if (this.plugins.has(plugin.name)) {
      return this as AstroKit<W>;
    }
